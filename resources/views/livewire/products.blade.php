<div>
    ...
    <div class="flex flex-col w-full gap-2 rounded-lg">
     ...
     alt="{{ $product->name }}">
                ...
                <div class="flex flex-row gap-3">
                    <button wire:click="openEditModal({{ $product->id }})" class="bg-blue-400 text-blue-950 text-sm p-2 hover:bg-blue-500 hover:text-white transition">Editar</button>
                    <button wire:click="openDeleteModal({{ $product->id }})" class="bg-red-400 text-red-950 text-sm p-2 hover:bg-red-500 hover:text-white transition">Excluir</button>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-zinc-900 p-6 rounded-xl shadow-xl text-center flex flex-col items-center w-96">
            ...
            <p class="text-zinc-400 text-sm mb-3">Altere os dados que deseja alterar</p>
            ...
            <div class="flex flex-col gap-3 w-full">
                ...
                <div class="flex flex-col w-full gap-1">
                    <label for="image" class="flex mb-2 text-sm font-medium text-gray-900 dark:text-white">Foto atual do produto</label>
                    
                    <div class="flex flex-row justify-between">
                        <img class="h-24 w-24 object-cover rounded" src="{{ $imagePath === 'img/no-photo.png' ? asset('img/no-photo.png') : asset('storage/' . $imagePath) }}" alt="{{ $name }}">
                        <x-fas-arrow-right class="w-5 text-white" />
                    @if ($image)
                        <img class="h-24 w-24 object-cover rounded" src="{{ $image->temporaryUrl() }}" alt="Nova imagem">
                    @else
                        <img class="h-24 w-24 object-cover rounded" src="{{ $imagePath === 'img/no-photo.png' ? asset('img/no-photo.png') : asset('storage/' . $imagePath) }}" alt="{{ $name }}">
                    @endif
                    </div>
                ...
                <div class="flex flex-col w-full gap-1">
                    <label for="image" class="flex mb-2 text-sm font-medium text-gray-900 dark:text-white">Alterar foto do produto</label>
                    <input name="image" wire:model="image" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_help" id="image" type="file" accept="image/png, image/gif, image/jpeg, image/jpg, image/webp">
    ...
    <div class="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-zinc-900 p-6 rounded-xl shadow-xl text-center flex flex-col items-center w-96">
            ...
            <p class="text-zinc-400 text-sm mb-3">Tem certeza que deseja excluir este produto?</p>
